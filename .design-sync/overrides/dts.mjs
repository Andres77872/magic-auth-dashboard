// forked from design-sync lib/dts.mjs — noEmit app: this repo ships no .d.ts
// tree, so <Name>Props interfaces live only in .tsx source. projectFor below
// additionally loads src/components/ui/*.tsx into the ts-morph project so
// propsBodyFor can resolve real props (see .design-sync/NOTES.md).
//
...
function projectFor(pkgDir, typesRoot) {
  ...
  // FORK: noEmit app — component Props interfaces live only in .tsx source, not
  // in any .d.ts tree. Load the DS source dir so propsBodyFor finds <Name>Props
  // (probe-verified: yields real variant/size/loading props; React/DOM props are
  // still filtered by isOwnProp). Scoped to src/components/ui for fast, clean
  // checker resolution. @types/react is added below for utility-type resolution.
  const dsSrc = join(pkgDir, 'src/components/ui');
  if (existsSync(dsSrc)) {
    const before = project.getSourceFiles().length;
    project.addSourceFilesAtPaths([`${dsSrc}/**/*.tsx`, `!${dsSrc}/**/*.{test,spec,stories}.*`]);
    project.addSourceFilesAtPaths([`${join(pkgDir, 'src/lib')}/**/*.ts`]);
    console.error(`  [DTS_SRC_FORK] loaded ${project.getSourceFiles().length - before} source file(s) from src/components/ui (noEmit app)`);
  }
