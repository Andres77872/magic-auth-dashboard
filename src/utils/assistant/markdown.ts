...
function makeLink(rawHref: string, label: string, depth: number): MdInline {
 ...
 */
function firstInline(text: string, depth: number): InlineMatch | null {
  let best: InlineMatch | null = null;
  const add = (
    re: RegExp,
    build: (m: RegExpExecArray) => MdInline,
    endOf?: (m: RegExpExecArray) => number
  ): void => {
    ...
    if (best === null || m.index < best.start) {
      best = {
        start: m.index,
        end: endOf ? endOf(m) : m.index + m[0].length,
        make: () => build(m),
      };
    }
  };
  ...
  add(/(`+)([\s\S]*?)\1/, (m) => ({
    type: 'code',
    value: normalizeCodeSpan(m[2]),
  }));
  // [label](href "title") and its image form (rendered as its alt text).
  add(/(!?)\[([^\]]*)\]\(\s*(<[^>]*>|[^)\s]*)(?:\s+"[^"]*")?\s*\)/, (m) =>
    m[1] === '!' ? { type: 'text', value: m[2] } : makeLink(m[3], m[2], depth)
  );
  add(/\*\*(\S(?:[\s\S]*?\S)?)\*\*/, (m) => ({
    type: 'strong',
    children: parseInline(m[1], depth + 1),
  }));
  add(/(?<![\p{L}\p{N}_])__(\S(?:[\s\S]*?\S)?)__(?![\p{L}\p{N}_])/u, (m) => ({
    type: 'strong',
    children: parseInline(m[1], depth + 1),
  }));
  add(/~~(\S(?:[\s\S]*?\S)?)~~/, (m) => ({
  ...
  add(/(?<![\p{L}\p{N}_])_(\S(?:[\s\S]*?\S)?)_(?![\p{L}\p{N}_])/u, (m) => ({
    type: 'em',
    children: parseInline(m[1], depth + 1),
  }));
...
export function parseInline(text: string, depth = 0): MdInline[] {
  ...
  while (rest.length && guard++ < 10_000) {
    const match = firstInline(rest, depth);
...
function inlineWithBreaks(lines: string[]): MdInline[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (i < lines.length - 1)
      out.push(hard ? { type: 'br' } : { type: 'text', value: ' ' });
  }
  return out;
}
...
function parseList(
  ...
  while (i < lines.length) {
    ...
    const contentIndent =
      marker[1].length + marker[2].length + marker[3].length;
...
export function parseMarkdown(source: string): MdBlock[] {
  if (source.length > 250000)
    return [{ type: 'code', value: source, lang: 'text', closed: true }];
