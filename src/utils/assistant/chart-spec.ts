...
export function sanitizeChartText(value: string, max: number): string {
  ...
  for (const character of value) {
    scrubbed += isDisplayHostile(character.codePointAt(0) ?? 0)
      ? ' '
      : character;
  }
 ...
 */
function parseLenientJson(source: string): unknown {
...
function draftFromRows(rows: unknown[]): DraftData | ChartSpecError {
  ...
  rows.forEach((row, index) => {
    if (Array.isArray(row)) {
      // ["kimi-k2", 1234]
      labels.push(
        sanitizeChartText(String(row[0] ?? index + 1), MAX_LABEL_CHARS)
      );
  ...
  if (series.length === 0) {
    return {
      ok: false,
      error: 'The chart has no numeric values.',
      hint: 'Give each row a "value", for example {"label":"kimi-k2","value":1234}.',
    };
  }
...
function draftFromSeries(
  rawSeries: unknown[],
  rawLabels: string[] | null
): DraftData | ChartSpecError {
  const series: DraftSeries[] = [];
  ...
  for (const [index, entry] of rawSeries.entries()) {
    ...
    if (!isRecord(entry)) {
      return {
        ok: false,
        error: 'Each series must be an object with a name and values.',
        hint: '{"name":"Requests","values":[12,9,4]}',
      };
    }
    const name =
      optionalText(
        firstKey(entry, ['name', 'label', 'id', 'key']),
        MAX_NAME_CHARS
      ) ?? `Series ${index + 1}`;
    ...
    if (!Array.isArray(rawValues)) {
      return {
        ok: false,
        error: `Series "${name}" has no values array.`,
        hint: '{"name":"Requests","values":[12,9,4]}',
      };
    }
    ...
    const values = rawValues.map((point, pointIndex) => {
      if (isRecord(point)) {
        ...
        return coerceValue(
          valueKey === undefined ? undefined : point[valueKey]
        );
      }
      if (Array.isArray(point)) {
        pointLabels[pointIndex] = sanitizeChartText(
          String(point[0] ?? pointIndex + 1),
          MAX_LABEL_CHARS
        );
    ...
    if (labels === null && pointLabels.length === values.length)
      labels = pointLabels;
  ...
  if (series.length === 0) {
    return {
      ok: false,
      error: 'The chart has no series.',
      hint: '"series":[{"name":"Requests","values":[12,9,4]}]',
    };
  }
  const width = Math.max(...series.map((s) => s.values.length));
  return {
    labels: labels ?? Array.from({ length: width }, (_, i) => String(i + 1)),
    series,
  };
}

function readLabels(root: Record<string, unknown>): string[] | null {
  const raw = firstKey(root, [
    'labels',
    'categories',
    'x',
    'buckets',
  ...
  return raw.map((entry, index) =>
    sanitizeChartText(
      entry === null || entry === undefined ? String(index + 1) : String(entry),
      MAX_LABEL_CHARS
    )
  );
}
...
export function parseChartSpec(
  ...
  if (!isRecord(root)) {
    return {
      ok: false,
      error: 'A chart block must contain a single JSON object.',
      hint: null,
    };
  }

  const resolved = resolveKind(
    firstKey(root, ['type', 'chart', 'kind', 'chart_type']),
    fenceKindHint(info)
  );
  if (resolved === null) {
    return {
...
function finalize(
  ...
  if (labels.length === 0) {
    return {
      ok: false,
      error: 'The chart has no categories to plot.',
      hint: null,
    };
  }
  ...
  if (labels.length * draft.series.length > MAX_CELLS) {
    return {
      ok: false,
      error: 'The chart has too many values to render.',
      hint: 'Reduce the buckets or the series count.',
    };
  }
  ...
  const unit = resolveUnit(
    firstKey(root, ['unit', 'units', 'metric_unit', 'value_unit'])
  );
  const spec: ChartSpec = {
    kind,
    stacked,
    title: optionalText(
      firstKey(root, ['title', 'caption', 'heading']),
      MAX_TITLE_CHARS
    ),
    unit,
    labels,
    series,
    source: optionalText(
      firstKey(root, ['source', 'source_id', 'data_source']),
      MAX_META_CHARS
    ),
    period: optionalText(
      firstKey(root, ['period', 'range', 'window', 'timeframe']),
      MAX_META_CHARS
    ),
    note: optionalText(
      firstKey(root, ['note', 'notes', 'caveat', 'footnote', 'provenance']),
      MAX_NOTE_CHARS
    ),
    hasUnknown,
  };
...
function validateForKind(spec: ChartSpec): ChartSpecResult {
  if (spec.kind === 'donut') {
    ...
    if (spec.series[0].values.some((value) => value !== null && value < 0)) {
      return {
        ok: false,
        error: 'A donut chart cannot show negative shares.',
        hint: 'Use a bar chart for values that can be negative.',
      };
    }
  ...
  if (
    spec.kind === 'heatmap' &&
    spec.series.some((s) =>
      s.values.some((value) => value !== null && value < 0)
    )
  ) {
    return {
      ok: false,
      error: 'A heatmap cannot show negative values.',
      hint: 'Use a bar or column chart instead.',
    };
  }
  if (
    spec.stacked &&
    ...
    return {
      ok: false,
      error: 'A stacked chart cannot mix negative values.',
      hint: 'Drop "stacked" so the series are compared side by side.',
    };
  }
  return { ok: true, spec };
}
...
export function chartProvenance(spec: ChartSpec): string | null {
  const parts = [spec.source, spec.period].filter(
