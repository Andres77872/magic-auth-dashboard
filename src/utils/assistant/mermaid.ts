...
function cleanLabel(raw: string): string {
  ...
  const collapsed = scrubbed
    .replace(/<br\s*\/?>/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  const unquoted = collapsed.replace(/^"(.*)"$/s, '$1').trim();
  return unquoted.length > MAX_LABEL_CHARS
    ? `${unquoted.slice(0, MAX_LABEL_CHARS - 1)}…`
    : unquoted;
}
...
export function parseMermaid(source: string): DiagramResult {
  ...
  if (lines.length === 0) {
    return {
      ok: false,
      error: 'The diagram block is empty.',
      hint: 'Start it with "flowchart TD", "sequenceDiagram", or "pie".',
    };
  }
  if (lines.length > MAX_LINES) {
    return {
      ok: false,
      error: `The diagram has ${lines.length} statements; the limit is ${MAX_LINES}.`,
      hint: null,
    };
  }
...
function readNodeRef(
  text: string,
  index: number
): { id: string; label: string | null; shape: NodeShape; end: number } | null {
...
function classifyLink(
  ...
  const raw = (
    pipeLabel ??
    (dashed ? inlineLabel.replace(/\s*\.$/, '') : inlineLabel) ??
    ''
  ).trim();
...
function parseFlowchart(
  ...
  const declare = (
    ...
    nodes.set(id, {
      id,
      lines: wrapLabel(cleanLabel(label ?? id)),
      shape,
      w: 0,
      h: 0,
    });
  ...
  for (const line of lines) {
    ...
    if (/^(?:style|classDef|class|linkStyle|click)\b/i.test(text)) {
      warn(
        'Styling and interaction directives are ignored; the dashboard theme paints the diagram.'
      );
      continue;
    }
    if (/^subgraph\b/i.test(text) || /^end$/i.test(text)) {
      warn(
        'Subgraph grouping is not drawn; its nodes appear in the main graph.'
      );
      continue;
    }
    ...
    if (pending !== null) {
      return {
        ok: false,
        error: `Line ${line.number} ends with a link that has no target.`,
        hint: null,
      };
    }
    ...
    if (nodes.size > MAX_NODES) {
      return {
        ok: false,
        error: `The diagram has more than ${MAX_NODES} nodes.`,
        hint: 'Summarize the structure or split it into two diagrams.',
      };
    }
    if (edges.length > MAX_EDGES) {
      return {
        ok: false,
        error: `The diagram has more than ${MAX_EDGES} links.`,
        hint: 'Summarize the structure or split it into two diagrams.',
      };
    }
  }
  ...
  if (nodes.size === 0) {
    return {
      ok: false,
      error: 'The flowchart declares no nodes.',
      hint: 'Add statements such as "A[Start] --> B[End]".',
    };
  }

  return {
    ok: true,
    diagram: layoutFlowchart([...nodes.values()], edges, direction),
    warnings,
  };
}
...
function sizeNode(node: RawNode): void {
  ...
  node.w = Math.min(
    NODE_MAX_W + 40,
    Math.max(NODE_MIN_W, Math.round(textWidth + NODE_PAD_X * 2 + extra))
  );
  node.h =
    Math.max(NODE_MIN_H, node.lines.length * LINE_HEIGHT + 16) +
    (node.shape === 'diamond' ? 14 : 0);
...
function borderPoint(
  node: { x: number; y: number; w: number; h: number; shape: NodeShape },
  dx: number,
  dy: number
): [number, number] {
...
function layoutFlowchart(
  ...
  for (const index of sortedRanks) {
    ...
    const depth = Math.max(
      ...group.map((node) => (horizontal ? node.w : node.h))
    );
  ...
  for (const index of sortedRanks) {
    const group = ranks.get(index) ?? [];
    const span =
      group.reduce((sum, node) => sum + (horizontal ? node.h : node.w), 0) +
      SIBLING_GAP * Math.max(0, group.length - 1);
    let cursor = 0;
    for (const node of group) {
      const size = horizontal ? node.h : node.w;
      positions.set(node.id, {
        rankAxis: rankOffset.get(index) ?? 0,
        crossAxis: cursor + size / 2 - span / 2,
      });
  ...
  const nodes: DiagramNode[] = rawNodes.map((node) => {
    ...
    const rankAxis = flipRank
      ? rankTotal - position.rankAxis
      : position.rankAxis;
  ...
  for (const edge of rawEdges) {
    ...
    edges.push({
      ...
      labelW:
        edge.label === null ? 0 : Math.round(measureText(edge.label, 10) + 10),
    });
  }
...
const SEQUENCE_MESSAGE =
  /^([A-Za-z0-9_]+)\s*(-{1,2}>>?|-{1,2}x|-{1,2}\))\s*([A-Za-z0-9_]+)\s*:\s*(.*)$/;
const SEQUENCE_NOTE = /^note\s+(left of|right of|over)\s+([^:]+):\s*(.*)$/i;
const SEQUENCE_PARTICIPANT =
...
function parseSequence(lines: SourceLine[]): DiagramResult {
  ...
  for (const line of lines) {
    const text = line.text;
    if (/^(?:activate|deactivate|autonumber|box|rect|link|links)\b/i.test(text))
      continue;
    if (
      /^(?:loop|alt|else|opt|par|and|critical|break)\b/i.test(text) ||
      /^end$/i.test(text)
    ) {
      warn(
        'Grouping blocks (loop, alt, opt) are not drawn; their messages appear in order.'
      );
      continue;
    }
    ...
    if (note) {
      ...
      notes.push({
        anchors,
        lines: wrapLabel(cleanLabel(note[3]), 28, 2),
  ...
  if (order.length === 0) {
    return {
      ok: false,
      error: 'The sequence diagram has no participants.',
      hint: 'Add messages such as "web->>backend: POST /chat".',
    };
  }
  if (order.length > MAX_ACTORS) {
    return {
      ok: false,
      error: `The sequence diagram has ${order.length} participants; the limit is ${MAX_ACTORS}.`,
      hint: null,
    };
  }
  if (messages.length > MAX_MESSAGES) {
    return {
      ok: false,
      error: `The sequence diagram has ${messages.length} messages; the limit is ${MAX_MESSAGES}.`,
      hint: null,
    };
  }
  ...
  return {
    ok: true,
    diagram: layoutSequence(order, labels, messages, notes),
    warnings,
  };
}
...
function layoutSequence(
  ...
  const emitNotes = (after: number): void => {
    for (const note of rawNotes.filter(
      ...
      const textWidth = Math.max(
        ...note.lines.map((line) => measureText(line, 11))
      );
...
function parsePie(header: string, lines: SourceLine[]): DiagramResult {
  ...
  for (const line of lines) {
    ...
    if (!Number.isFinite(value) || value < 0) {
      return {
        ok: false,
        error: `Line ${line.number} has a value that is not a non-negative number.`,
        hint: null,
      };
    }
  ...
  if (slices.length === 0) {
    return {
      ok: false,
      error: 'The pie diagram has no slices.',
      hint: 'Add lines such as "kimi-k2" : 62.',
    };
  }
