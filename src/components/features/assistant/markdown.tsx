...
import {
  ...
  type MdBlock,
  type MdInline,
} from '@/utils/assistant/markdown';
import { isChartFence } from '@/utils/assistant/chart-spec';
import { isDiagramFence } from '@/utils/assistant/mermaid';
...
function FencedBlock({
  ...
  if (isChartFence(lang))
    return <ChartArtifact source={value} info={lang} closed={closed} />;
  if (isDiagramFence(lang))
    return <DiagramArtifact source={value} info={lang} closed={closed} />;
...
function StreamCursor(): ReactNode {
  return (
    <span className="analytics-assistant-stream-cursor" aria-hidden="true" />
  );
}
              ...
              {block.rows.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {Array.from({ length: columns }, (_, colIndex) => (
                    <td
                      key={colIndex}
                      style={{ textAlign: block.align[colIndex] ?? undefined }}
                    >
...
export function Markdown({
  ...
  const inlineCaretTarget =
    blocks[lastIndex]?.type === 'paragraph' ||
    blocks[lastIndex]?.type === 'heading';
  return (
    <div className="analytics-assistant-md">
      {blocks.map((block, index) => (
        <BlockNode
          ...
          block={block}
          trailing={
            streaming && inlineCaretTarget && index === lastIndex ? (
              <StreamCursor />
            ) : undefined
          }
        />
