...
} from '@/utils/assistant/mermaid';
import type { ChartSpec } from '@/utils/assistant/chart-spec';
...
function NodeShape({ node }: { node: DiagramNode }): ReactNode {
  const { x, y, w, h } = node;
  const common = {
...
function FlowchartView({ diagram }: { diagram: FlowchartDiagram }): ReactNode {
  return (
    <svg
...
function SequenceView({ diagram }: { diagram: SequenceDiagram }): ReactNode {
  return (
    <svg
      className="md-diagram-svg"
      viewBox={`0 0 ${diagram.width} ${diagram.height}`}
...
export function DiagramArtifact({
  ...
  const result = useMemo(
    () => (closed ? parseMermaid(source) : null),
    [source, closed]
  );
  ...
  if (result.diagram.kind === 'pie') {
    return (
      <ChartFigure
        spec={pieToChartSpec(result.diagram)}
        source={source}
        kind="Diagram"
      />
    );
