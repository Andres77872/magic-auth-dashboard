...
export function AdminProjectsManager({
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      const adminProjectsResponse =
        await userService.getAdminProjects(userHash);
      const currentAssigned =
        adminProjectsResponse.success && adminProjectsResponse.assigned_projects
      ...
      if (allProjectsResponse.success) {
        ...
        const assignedIds = currentAssigned.map((p: any) =>
          String(p.project_id)
        );
        setAvailableProjects(
          projectsList.filter(
            (p: any) => !assignedIds.includes(String(p.project_id))
          )
        );
      }
    } catch (error) {
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent size="lg" className="admin-projects-manager-modal">
          <DialogHeader>
            <DialogTitle>Manage Projects for {username}</DialogTitle>
          </DialogHeader>
                  ...
                  <div className="admin-projects-grid">
                    {assignedProjects.map((project) => (
                      <div
                        ...
                        className="project-card assigned"
                      >
                ...
                {availableProjects.length === 0 ? (
                  ...
                  <div className="admin-projects-grid">
                    {availableProjects.map((project) => (
                      <div
                        key={project.project_id}
                        className="project-card available"
                      >
                        <div className="project-card-header">
                          <FolderKanban size={16} />
                          <div className="project-info">
