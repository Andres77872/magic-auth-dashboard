...
export function ProjectAnalyticsDashboard(): React.JSX.Element {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to load project analytics'
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Monitor project health, activity, and member engagement
              </CardDescription>
            </div>
          </div>
        ...
        {data.engagement && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="p-4 rounded-lg border border-border">
              ...
              <div className="space-y-2">
                {data.engagement.popularActions
                  .slice(0, 5)
                  .map((action, index) => (
                    <div
                      key={index}
            ...
            <div className="flex items-end gap-1 h-32">
              {data.engagement.projectActivity.map((day, index) => {
                const maxCount = Math.max(
                  ...data.engagement!.projectActivity.map(
                    (d) => d.activityCount
                  )
                );
                const height =
                  maxCount > 0 ? (day.activityCount / maxCount) * 100 : 0;
