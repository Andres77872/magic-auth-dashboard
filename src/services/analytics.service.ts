...
export class AnalyticsService {
      ...
      const activities: Activity[] = activitiesArray.map((item: any) => ({
        ...
        description:
          typeof item.details === 'string'
            ? item.details
            : item.details?.message || 'Activity recorded',
  ...
  async getUserAnalytics(dateRange?: {
    ...
    const days = dateRange?.start
      ? Math.ceil(
          (new Date().getTime() - new Date(dateRange.start).getTime()) /
            (1000 * 60 * 60 * 24)
        )
      : 30;
    ...
    try {
      ...
      const [statsResponse, dashboardResponse] = await Promise.all([
        apiClient
          .get<any>('/admin/users/statistics', { days })
          .catch(() => ({ data: null })),
        apiClient
          .get<any>('/admin/dashboard/stats')
          .catch(() => ({ data: null })),
      ]);
      ...
      return {
        metrics: {
          ...
          newUsersThisWeek:
            ...
            dashboard.recent_activity?.new_users_7d ||
            0,
          userGrowthRate: stats.growth_rate_percentage || 0,
          usersByType: {
            root:
              stats.user_type_breakdown?.root ||
              dashboard.user_breakdown?.root_users ||
              0,
            admin:
              stats.user_type_breakdown?.admin ||
              dashboard.user_breakdown?.admin_users ||
              0,
            consumer:
              stats.user_type_breakdown?.consumer ||
              dashboard.user_breakdown?.consumer_users ||
              0,
          },
        },
        engagement: {
  ...
  async getProjectAnalytics(dateRange?: {
    ...
    try {
      const response = await apiClient.get<any>('/admin/projects/statistics', {
        days,
      });
      const stats = response.data?.statistics || {};

      return {
        projects: (stats.most_active_projects || []).map(
          (p: any, index: number) => ({
            id: String(index),
            name: p.project_name,
        ...
        avgHealthScore: 85,
        totalActivity:
          stats.most_active_projects?.reduce(
            (acc: number, p: any) => acc + (p.activity_count || 0),
            0
          ) || 0,
        engagement: {
          projectActivity: [],
  ...
  async getProjectAnalyticsById(
    ...
    try {
      ...
      console.error(
        `Failed to fetch analytics for project ${projectHash}:`,
        error
      );
      throw new Error('Failed to load project analytics');
    }
